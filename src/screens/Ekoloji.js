  ...
  const fetchPosts = async (pageNumber = 1) => {
    try {
      const response = await axios.get(
        `https://yeniyasamgazetesi9.com/wp-json/wp/v2/posts?categories=53&_embed&page=${pageNumber}`
      );
      ...
      console.error('Ekoloji verisi alınamadı:', error);
    } finally {
      setLoading(false);
      ...
      >
        {posts.map((item) => (
          ...
          >
            ...
            <Text style={[styles.postExcerpt, { color: isDarkMode ? '#ccc' : '#444' }]}> {item.excerpt} </Text>
          </TouchableOpacity>
        ))}
